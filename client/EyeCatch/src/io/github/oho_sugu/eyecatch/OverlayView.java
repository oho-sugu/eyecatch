...
public class OverlayView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		synchronized (listResult) {
			...
			if (this.listResult != null) {
				...
				int i = 0;
				if (listResult.keywords != null) {
					for (ListResult.Keyword keyword : listResult.keywords) {
						drawTextWithBorder(canvas, keyword.keyword + " "
								+ keyword.count, 250, 32 * i * 32, mainPaint,
								backPaint);
						i++;
					}
				}
	...
	private void drawTextWithBorder(Canvas canvas, String text, int x, int y,
			Paint fore, Paint back) {
	...
	synchronized public void updateKeywords(ListResult listResult) {
